...
export class GameEngine {
  ...
  private isMovePossible(piece: BoardPiece, destination: Position): boolean {
    ...
    if (distance > 1) {
      ...
      const inBetweenPosition = this.getInBetweenPosition(
        piece.position,
        destination,
      );
...
export class BoardDimensions {
  ...
  public isBlockedPosition(p: Position): boolean {
    return !!this.blockedPositions.find(
      (value) => value.row === p.row && value.col === p.col,
    );
  }
  ...
  private getBlockedPositions(): Position[] {
    ...
    for (let i = this.cornerSize; 0 < i; i--) {
      for (let j = i; 0 < j; j--) {
        ...
        positions.push(
          new Position(
            this.height - zeroAdjustedRow,
            this.width - zeroAdjustedCol,
          ),
        );
        positions.push(
          new Position(this.height - zeroAdjustedRow, zeroAdjustedCol),
        );
        positions.push(
          new Position(zeroAdjustedRow, this.width - zeroAdjustedCol),
        );
...
export class GameInstance {
  ...
  public static claudioHalma(players: string[]): GameInstance {
    ...
    const gameState = GameStateUtil.initClaudioHalmaBoard(
      players,
      boardDimensions,
    );
...
export class GameStateUtil {
  public static initClaudioHalmaBoard(
    players: string[],
    boardDimensions: BoardDimensions,
  ): GameState {
    ...
    if (players.at(0)) {
      ...
      for (
        let col = boardDimensions.cornerSize;
        col < boardDimensions.width - boardDimensions.cornerSize;
        col++
      ) {
    ...
    if (players.at(1)) {
      ...
      for (
        ...
        pieces.push(
          new BoardPiece(new Position(zeroAdjustedHeight, col), players[1]),
        );
        pieces.push(
          new BoardPiece(new Position(zeroAdjustedHeight - 1, col), players[1]),
        );
      }
    }
    ...
    if (players.at(2)) {
      // Fill pieces for player 1
      for (
        let row = boardDimensions.cornerSize;
        row < boardDimensions.height - boardDimensions.cornerSize;
        row++
      ) {
    ...
    if (players.at(3)) {
      ...
      for (
        ...
        pieces.push(
          new BoardPiece(new Position(row, boardDimensions.width), players[3]),
        );
        pieces.push(
          new BoardPiece(
            new Position(row, boardDimensions.width - 1),
            players[3],
          ),
        );
      }
